import { handleDsNoteEvents } from '../../src/transformers/common'
...
describe('handleDsNoteEvents', () => {
  ...
  it('works with ver 2 logs', async () => {
    ...
    const handlers = {
      'tend(uint256,uint256,uint256)': sinon.spy(),
    }

    await handleDsNoteEvents(services, flapperAbi, logs, handlers, 2)

    expect(handlers['tend(uint256,uint256,uint256)']).to.be.calledOnce
  })
  ...
  it('should work with ver 1 logs', async () => {
    ...
    const handlers = {
      'lock(uint256)': sinon.spy(),
    }

    await handleDsNoteEvents(services, dsChiefAbi, logs, handlers)

    expect(handlers['lock(uint256)']).to.be.calledOnce
  })
  ...
  it('wont crash with non DSNote events', async () => {
    ...
    expect(handlers['lock(uint256)']).to.not.been.calledOnce
  })
})
