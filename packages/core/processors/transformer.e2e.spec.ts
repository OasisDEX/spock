...
`);
  ...
  it('should work with transformers with extractor dependencies that were archived', async () => {
    const dbCtx = createDB(testConfig.db);
    await prepareDB(dbCtx.db, testConfig);
    ...
    const services: Services = {
      ...
      config: {
        ...testConfig,
        extractors: [{ name: 'test-extractor' } as any],
...
`);
  ...
  it('should work with transformers with extractor and transformer dependencies that are archived', async () => {
    const dbCtx = createDB(testConfig.db);
    await prepareDB(dbCtx.db, testConfig);
